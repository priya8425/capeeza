...
?>
<?php session_start();
$res=mysqli_query($conn,"SELECT * FROM `adminllogin`");
...
<?php
		...
		if(isset($_POST['forget'])){
			...
			$email=$_POST['mail'];
			$q=mysqli_query($conn,"select * from adminllogin where email='$email'");
        ...
        //TCP port to connect to; use 587 if you have set `SMTPSecure = PHPMailer::ENCRYPTION_STARTTLS`
				...
				//Recipients
				$mail->setFrom('divyani.keshari@tectignis.in', 'Capeeza ENTERPRISES');
				$mail->addAddress($email, 'Capeeza ENTERPRISES');     //Add a recipient
				...
				if($mail->send()){
					...
					$q1=mysqli_query($conn,"update adminllogin set password= '$passwordhash' where email='$email'");
						if($q1){
...
<head>
  ...
  <title>Capeeza | FORGIOT PASSWORD</title>
